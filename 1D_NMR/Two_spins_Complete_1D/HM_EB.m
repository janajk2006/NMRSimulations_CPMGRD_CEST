...
detop_EB=H_V'*detop*H_V;		% convert the detection operator to Eigen basis using Eigen vectors (E_V)
					% obtained by diagnolising the hamiltonion in earlier step
...
Pulse_left_EB = H_V'*Pulse_left*H_V;
Pulse_right_EB = H_V'*Pulse_right*H_V;

%sigmap_y=Pulse_left*sigma0*Pulse_right;	% First pulse
sigma0_EB=H_V'*sigma0*H_V;	% convert the density matrix to Eigen basis using Eigen vectors (E_V)
sigmap_y_EB=(Pulse_left_EB*sigma0_EB*Pulse_right_EB); % Apply 90 degree puse along y to both spins that are coupled
