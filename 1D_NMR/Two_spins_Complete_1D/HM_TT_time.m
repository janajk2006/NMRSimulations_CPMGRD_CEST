...
for j=1:Dim
	...
	%fprintf("[%d,%d]: %f %f %f\n",i,j,Ttable(T_index,1),Ttable(T_index,2),Ttable(T_index,3));
end % col index
end % row index
...
for i=1:Dim*Dim
for j=1:npts
	FID(1,j)=FID(1,j)+(Ttable(i,1)*power(exp(2*pi*complex(0,1)*Ttable(i,2)*td),j-1)*Ttable(i,3))*power(T_relax,j);
end % j (fid)
end % i (transition)
...
outfile=fopen("Results_HM_TT_time","w");
for i=1:npts
