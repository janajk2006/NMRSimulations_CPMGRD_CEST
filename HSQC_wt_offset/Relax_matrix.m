...
% by Jana.K
% Ackowledgement:  T. S. Mahesh, Ilya Kuprov's 
% ============================================================================================

function [L_DD,L_CC,L_DC]=Relax_matrix(v_ppm,Secular_approx,J_H1N2,n_spins,MHZ_H,v_iso,v_delzz,v_eta,Coord_1,Coord_2,EA_1,EA_2,S,Tau);
...
Gamma_1=Rfreq.*Gamma_H/Rfreq_H; 
MHZ=(Gamma_1.*(MHZ_H/Gamma_H));
...
H_csham=-(v_Hz(1,1)*HzNi); % Chemical Shift Hamiltonian 
N_csham=-(v_Hz(1,2)*HiNz); % Chemical Shift Hamiltonian 
...
for j=i+1:n_spins
	...
	%printf("[%d] tau[i]: %0.12f omega: %0.12f c1[i]: %0.12f c2[i]: %0.12f J: %0.12f\n",m,Tau_eff(1,m),wtr,c1s(1,m),c1s(1,m),J(l1,l2));
			...
			% calculate the element of L_DD super operator
			aaa=1;
			...
			for aa=1:Dim	
				...
				L_DD(aaa,bbb)=L_DD(aaa,bbb)+Rel;
				
				% printf("LR[%d,%d,%d,%d,%d,%d]:%0.6f+%0.6f \n",a,aa,b,bb,aaa,bbb,real(L_DD(aaa,bbb)),imag(L_DD(aaa,bbb)));  					%pause
...
HC=-(v_Hz(1,1)+MHZ_H*power(10,6)*(Gamma_1(1,1)/Gamma_H))*HzNi - (v_Hz(1,2)+MHZ_H*power(10,6)*Gamma_1(1,2)/Gamma_H)*HiNz;
...
% L_H0_EB=L_H0_EB-(L_DC+L_DD+L_CC);	% Sign convention..Chemical exchange added (sign absorbed into kinetic matrix) but relaxation is substracted

L_Relax=L_DD+L_CC-L_DC;
