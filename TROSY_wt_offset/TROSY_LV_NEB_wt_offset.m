...
L_H0_11=-complex(0,1)*2*pi*(diprod(H0_1,L_i)-diprod(L_i,H0_1));
L_H0_11=diprod(Comp_I,L_H0_11);
...
L_H0_22=-complex(0,1)*2*pi*(diprod(H0_2,L_i)-diprod(L_i,H0_2));
L_H0_22=diprod(Comp_II,L_H0_22);
...
U_left_t1 = expm(-L_H00_final*(dw1/2)); 	% for correct CS sign of H0_decp_final is reversed 
						% (probably bcs in expt 180 pulse in middle of t1 will reverse the sign of coherence)
%U_left_t2 = expm(-L_H0_decp_final*dw2);
U_left_t2 = expm(-L_H00_final*dw2);
	...
	File_Result=sprintf('./NMRPipe/TROSY_15NH_LV_NEB_wt_offset_%.0f_kex_%.0fA-%.0fB_Pop.asc',K,Pop_A,Pop_B);
...
% NMRPipe Script
File_str=sprintf('TROSY_15NH_LV_NEB_wt_offset_%.0f_kex_%.0fA-%.0fB_Pop',K,Pop_A,Pop_B);
fprintf(outfile_2,'\n# Expt: 1');
fprintf(outfile_2,'\n# -------');
