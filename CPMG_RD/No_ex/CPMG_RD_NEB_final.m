...
N_CPMG_PULSES=[2 4 6 8 12 16 20 24 32 64]; 	%  Each entry correspond to separate ext with the entry number as the 
...
Pop_B=100-Pop_A;
K=power(10,4); % set value as Fast: 100000, Intermediate: 500, Slow: 10 % Octave expm() hv issues with K>10K matlab doesnt
...
TR=(CPMG_N/J_HN);	% Total relaxation period
	...
	% NMR Pipe script
	mkdir('./NMRPipe');
	File_Result=sprintf('./NMRPipe/NMRpipe_script_cpmg');
